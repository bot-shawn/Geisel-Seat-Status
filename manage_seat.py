...
def get_lib_status():
    with sqlite3.connect(DB_PATH) as conn:
        ...
        cursor.execute('''
                    SELECT floor_id, floor_section, occupied_seats, total_seats, noise_level
                    FROM floor_status
                ''')        
...
def update_seats(floor_id, section_name, change):
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
...
# When its midnight we want to be able to reset everything. 
def reset_library():
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        # Set everyone to 0 at once!
        cursor.execute("UPDATE floor_status SET occupied_seats = 0, last_updated = ?", (datetime.now(),))
...
if __name__ == "__main__":
    ...
    print("\nAttempting to update Floor 2 East Wing")
    update_seats(2, 'Geisel East', -20)
